-- SQL queries to delete test matters
-- Use these if your backend uses SQL Server, Azure SQL, or similar database

-- Query 1: Delete by MatterID (if using MatterID as primary key)
DELETE FROM matters 
WHERE MatterID IN (
...
-- Query 2: Alternative - Delete by description pattern (more aggressive, be careful!)
DELETE FROM matters 
WHERE Description LIKE '%test%' 
   OR Description LIKE '%TEST%'
   OR Description = 'Fuga Inventore ipsa'
   OR Description LIKE '%test item%';

-- Query 3: Safe preview - See what would be deleted before running DELETE
SELECT MatterID, InstructionRef, Description, OpenDate, ResponsibleSolicitor, Status
FROM matters 
WHERE MatterID IN (
    '049f6a74-9c25-4c62-9684-bc2af8b5a58a',
    '09031ba4-648d-42ff-a313-9a8aba014292', 
...
-- Query 4: Count how many test records exist before deletion
SELECT 
    COUNT(*) as TotalTestRecords,
    COUNT(CASE WHEN ResponsibleSolicitor = 'Lukasz Zemanek' THEN 1 END) as LukaszTestRecords
FROM matters 
WHERE Description LIKE '%test%' 
   OR Description LIKE '%TEST%'
   OR Description = 'Fuga Inventore ipsa'
