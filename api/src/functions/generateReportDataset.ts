...
interface WIP {
  ...
  updated_at?: string;
  type?: string;
  matter?: { id: number; display_number: string; etag?: string };
  quantity_in_hours?: number;
  note?: string;
  total?: number;
  price?: number;
  expense_category?: string | null;
  activity_description?: { id: number; name: string; etag?: string };
  user?: { id: number; redacted?: boolean };
  bill?: { id: number } | null;
  billed?: boolean;
}
...
interface OutstandingClientBalancesResponse {
  meta: { paging: any; records: number };
...
// Helper functions for assigning column values
...
function assignWIPColumnValue(row: WIP, colName: keyof WIP | string, value: any) {
  switch (colName) {
    case "id":
      row.id = value != null ? Number(value) : 0;
      break;
    case "date":
      row.date = value != null ? String(value) : undefined;
      break;
    case "created_at":
      row.created_at = value != null ? String(value) : undefined;
      break;
    case "updated_at":
      row.updated_at = value != null ? String(value) : undefined;
      break;
    case "type":
      row.type = value != null ? String(value) : undefined;
      break;
    case "note":
      row.note = value != null ? String(value) : undefined;
      break;
    case "expense_category":
      row.expense_category = value != null ? String(value) : null;
      break;
    case "quantity_in_hours":
      row.quantity_in_hours = value != null ? Number(value) : undefined;
      break;
    case "total":
      row.total = value != null ? Number(value) : undefined;
      break;
    case "price":
      row.price = value != null ? Number(value) : undefined;
      break;
    case "matter_id":
      row.matter = row.matter || { id: 0, display_number: "" };
      row.matter.id = value != null ? Number(value) : 0;
      break;
    case "matter_display_number":
      row.matter = row.matter || { id: 0, display_number: "" };
      row.matter.display_number = value != null ? String(value) : "";
      break;
    case "activity_description_id":
      row.activity_description = row.activity_description || { id: 0, name: "" };
      row.activity_description.id = value != null ? Number(value) : 0;
      break;
    case "activity_description_name":
      row.activity_description = row.activity_description || { id: 0, name: "" };
      row.activity_description.name = value != null ? String(value) : "";
      break;
    case "user_id":
      row.user = row.user || { id: 0 };
      row.user.id = value != null ? Number(value) : 0;
      break;
    case "bill_id":
      row.bill = value != null ? { id: Number(value) } : null;
      break;
    case "billed":
      row.billed = value != null ? Boolean(value) : undefined;
      break;
  }
}
...
export async function generateReportDatasetHandler(req: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  ...
  if (!datasets || !Array.isArray(datasets) || datasets.length === 0) {
    return { status: 400, body: "Missing or invalid 'datasets' array in request body." };
  }

  if (!entraId) {
    return { status: 400, body: "Missing 'entraId' in request body. A user is required via Teams context." };
  }
  ...
  if (!refreshToken || !clientSecretValue || !clientId) {
    ...
    return { status: 500, body: "Missing Clio OAuth credentials." };
  }
  ...
  try {
    ...
    clioWip = await fetchClioActivities(clioActivitiesUrl, startDate, endDate, accessToken, context);
    context.log(`Fetched ${clioWip.length} Clio WIP entries from ${startDate} to ${endDate}`);

    // Round up quantity_in_hours to one decimal place for Clio WIP
    clioWip = clioWip.map((entry) => ({
        ...entry,
        quantity_in_hours: entry.quantity_in_hours !== undefined ? Math.ceil(entry.quantity_in_hours * 10) / 10 : undefined,
    }));

    // Fetch SQL WIP for all available data (adjusted date range)
    const { dateFrom, dateTo } = getLast24MonthsExcludingCurrentWeek();
    context.log(`Fetching SQL WIP from ${dateFrom} to ${dateTo}`);
    sqlWip = await new Promise<WIP[]>((resolve, reject) => {
        const result: WIP[] = [];
        const query = `
        SELECT 
            id,
        ...
        const request = new SqlRequest(query, (err) => {
        if (err) {
            context.error("SQL Query Error (sqlWip):", err);
            reject(err);
        }
        });
        request.addParameter("DateFrom", TYPES.Date, dateFrom);
        ...
        if (row.quantity_in_hours !== undefined) {
            row.quantity_in_hours = Math.ceil(row.quantity_in_hours * 10) / 10;
        }
        result.push(row);
        });

        request.on("requestCompleted", () => {
        context.log(`Fetched ${result.length} SQL WIP entries`);
        resolve(result);
        });
        connection.execSql(request);
    });

    // Merge Clio and SQL WIP into a single wip array
    responseData.wip = [...clioWip, ...sqlWip];
    context.log(`Merged WIP: ${clioWip.length} Clio + ${sqlWip.length} SQL = ${responseData.wip.length} total entries`);
    }

    // Keep sqlWip separate if explicitly requested (optional for debugging)
    if (datasets.includes("sqlWip") && !datasets.includes("wip")) {
    const { dateFrom, dateTo } = getLast24MonthsExcludingCurrentWeek();
    context.log(`Fetching SQL WIP (standalone) from ${dateFrom} to ${dateTo}`);
    sqlWip = await new Promise<WIP[]>((resolve, reject) => {
        const result: WIP[] = [];
        const query = `
        SELECT 
        ...
        const request = new SqlRequest(query, (err) => {
        if (err) {
            context.error("SQL Query Error (sqlWip standalone):", err);
            reject(err);
        }
        });
        request.addParameter("DateFrom", TYPES.Date, dateFrom);
        ...
        request.on("requestCompleted", () => {
        context.log(`Fetched ${result.length} SQL WIP entries (standalone)`);
        resolve(result);
        });
        connection.execSql(request);
    });
    responseData.wip = sqlWip;
    }
...
function getLast24Months(): { dateFrom: string; dateTo: string } {
  ...
  const dateFrom = new Date(now.getFullYear(), now.getMonth() - 23, 1);
  return {
...
function getLast24MonthsExcludingCurrentWeek(): { dateFrom: string; dateTo: string } {
  const now = new Date();
  const bstOffset = 1 * 60 * 60 * 1000;
  const bstNow = new Date(now.getTime() + bstOffset);
  const weekStart = new Date(bstNow);
  const day = weekStart.getDay();
  const diff = day === 0 ? -6 : 1 - day;
  weekStart.setDate(weekStart.getDate() + diff);
  weekStart.setHours(0, 1, 0, 0);
  const endDate = new Date(bstNow); // Adjusted to current date for testing
  endDate.setHours(23, 59, 59, 999);
  const dateFrom = new Date(endDate);
...
async function fetchClioActivities(
  ...
  while (true) {
    ...
    const data: { data: WIP[]; meta: { paging: { next?: string; records: number } } } = await response.json();
