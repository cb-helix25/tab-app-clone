...
export async function getSnippetBlocksHandler(req: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    context.log("getSnippetBlocks invoked");
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return { status: 500, body: "Failed to fetch snippet blocks" };
        }
...
app.http("getSnippetBlocks", {
    methods: ["GET"],
    authLevel: "function",
    handler: getSnippetBlocksHandler,
});
