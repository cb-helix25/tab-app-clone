...
const formatCurrency = (amount: number): string => {
  if (amount < 10000) {
    return `┬ú${amount.toLocaleString('en-GB', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`;
  } else if (amount < 100000) {
    return `┬ú${(amount / 1000).toFixed(1)}k`;
  } else {
    return `┬ú${(amount / 1000).toFixed(2)}k`;
  }
};
  ...
  return (
    <div className="management-dashboard-container animate-dashboard">
      ...
      >
        ...
        <MetricCard title="WIP (┬ú)" value={formatCurrency(totalWipPoundsOverall)} style={{ flex: '1', minWidth: '200px' }} />
        <MetricCard
          title="Collected"
      ...
      >
        ...
        >
          ...
          >
            Person {sortKey === 'initial' ? (sortDirection === 'asc' ? 'Ôû▓' : 'Ôû╝') : ''}
          </div>
          {visibleColumns.map((colKey) => (
            ...
            >
              <span style={{ fontSize: '1.2em', fontWeight: 'bold' }}>
                {columnLabels[colKey]} {sortKey === colKey ? (sortDirection === 'asc' ? 'Ôû▓' : 'Ôû╝') : ''}
              </span>
            </div>
          ))}
        </div>
